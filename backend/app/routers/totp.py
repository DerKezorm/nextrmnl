...
from ..security import brake, verify_password
from ..services import settings_service, totp, vault
...
def confirm(payload: ConfirmIn, account: CurrentAccount, db: DbSession) -> dict[str, Any]:
    if account.totp_secret_enc:
        raise fehler("totp_enabled", "The second factor is already on. Turn it off first.", 409)
...
def disable(payload: PasswordIn, account: CurrentAccount, db: DbSession) -> dict[str, Any]:
...
def new_recovery_codes(payload: PasswordIn, account: CurrentAccount, db: DbSession) -> dict[str, Any]:
    if not account.totp_secret_enc:
        raise fehler("totp_not_enabled", "The second factor is not on.", 409)
    _check_password(account, payload.password)
    codes = totp.generate_recovery_codes()
...
def operator_reset(account_id: int, operator: OperatorAccount, db: DbSession) -> dict[str, Any]:
    ...
    if not row.totp_secret_enc:
        raise fehler("totp_not_enabled", "The second factor is not on.", 409)
    _reset(db, row)
    # Whoever held a session of that account keeps it; the sign-in rule changed, not the sessions. The operator
    # decides about those under Sessions.
    logger.warning("Second factor reset by operator account=%s by=%s", row.name, operator.name)
...
def login_code(payload: CodeIn, request: Request, response: Response, db: DbSession) -> dict[str, Any]:
    ...
    if len(typed) == totp.DIGITS and typed.isdigit():
        step = totp.verify_code(totp.open_seed(account.totp_secret_enc), typed, after_step=account.totp_last_step)
    ...
    if not accepted:
        brake.failed(key)
        still_pending = totp.fail_pending(token)
